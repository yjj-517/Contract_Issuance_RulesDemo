<template>
  <div class="PrivatePlacementContract">
    <div>{{ msg }}</div>
    <div class="PrivatePlacementContract-Box">
      <p class="PrivatePlacementContract-Box-first">
        <span>首次释放(10%) :</span>
      ...
      <p class="PrivatePlacementContract-Box-month">
        ...
        <el-input
          autosize
          class="PrivatePlacementContract-Box-month-textarea"
          type="textarea"
          disabled
          v-model="monthTime"
        ></el-input>
...
export default {
  name: "PrivatePlacementContract",
  // 模板引入
  components: {},
  // 数据
  data() {
    return {
      msg: "Welcome to Your Private Placement Contract Page",
      firstNum: "", // 首次释放数量
      monthNum: "", // 每月释放数量
      total: 0, // 每月释放总量
  ...
  methods: {
    ...
    async firstRelease() {
      ...
      // const PrivatePlacementContract = new ethers.Contract(
      //   seedRoundaddress,
      //   SeedRound.abi,
      //   signer
      // );
      ...
      // 调用智能合约获取首次释放
      let transaction = await PrivatePlacementContract.requestReleaseAll(
        overrides
      );
...
.PrivatePlacementContract-Box {
  width: 100%;
  height: 100%;
  margin-top: 2rem;
  display: flex;
  flex-direction: column;
  .PrivatePlacementContract-Box-first {
    display: flex;
    align-items: center;
    .el-input {
      width: 10rem;
  ...
  .PrivatePlacementContract-Box-month {
    display: flex;
    align-items: center;
    .el-input {
      width: 20rem;
    ...
    .PrivatePlacementContract-Box-month-textarea {
      width: 20rem;
      margin-left: 1rem;
    }
    .PrivatePlacementContract-Box-month-Btn {
      width: 4rem;
      height: 2.5rem;
      margin-left: 1rem;
    }
